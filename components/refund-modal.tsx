...
export function RefundModal({ balanceCents, onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      captureEvent('refund_requested', { amount: num })
      onSuccess?.()
      onClose()
    } catch {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto p-6 sm:p-8">
        <h2 className="text-2xl font-bold text-ink mb-2">Refund to card</h2>
        <p className="text-ink/70 mb-2">Available: ${maxAmount.toFixed(2)}</p>
        <p className="text-sm text-ink/60 mb-6">
          Unused balance is refunded to the card(s) used for deposits. It can take a few business
          days to appear on your statement.
        </p>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="refund-amount" className="block text-sm font-medium text-ink mb-2">
              Amount (NZD)
            </label>
            <input
              id="refund-amount"
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {loading ? 'Processing…' : 'Refund to card'}
            </button>
          </div>
        </form>
      </div>
