...
export default function ListerVerifyCompletionPage() {
  ...
  const handleVerify = async () => {
    ...
    setPhotoError(null)
    const supabase = createClient()
    const uploaded = await uploadCompletionPhotos(supabase, jobId, user.id, 'lister', photos)
    if (uploaded.error) {
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-canvas">
        <PageHero
          backHref="/dashboard/lister/jobs-listed"
          backLabel="Back to Active Jobs"
          title="Verify work complete"
          subtitle="Confirm the student has completed the job as agreed before releasing payment."
        />
